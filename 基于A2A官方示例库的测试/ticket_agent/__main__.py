...
def _build_mcp_configs() -> dict[str, Any]:
    """Builds the 12306 MCP server config from environment variables."""
    config: dict[str, Any] = {
        'command': os.getenv('TICKET_MCP_COMMAND', 'uvx'),
        'args': json.loads(
            os.getenv('TICKET_MCP_ARGS', '["mcp-server-12306"]')
        ),
        'transport': 'stdio',
    }
    env_json = os.getenv('TICKET_MCP_ENV_JSON')
    if env_json:
        config['env'] = json.loads(env_json)
    return {'ticket': config}


SERVER_CONFIGS = _build_mcp_configs()
...
def main(
    ...
    """Command Line Interface to start the Ticket Agent server."""
    ...
    async def run_server_async():
        async with app_lifespan(app_context):
            ...
            # Initialize TicketAgentExecutor with preloaded tools
            ticket_agent_executor = TicketAgentExecutor(
                mcp_tools=app_context.get('mcp_tools', [])
            )

            request_handler = DefaultRequestHandler(
                agent_executor=ticket_agent_executor,
...
def get_agent_card(host: str, port: int):
    """Returns the Agent Card for the Ticket Agent."""
    capabilities = AgentCapabilities(streaming=True, push_notifications=True)
    skill = AgentSkill(
        id='ticket_query',
        name='Query 12306 train tickets',
        description='Helps with querying 12306 train tickets for a date and route',
        tags=['12306', 'train ticket'],
        examples=[
            '2025年8月20日从北京到上海的高铁票'
        ],
    )
    ...
    return AgentCard(
        name='Ticket Agent',
        description='Helps with querying 12306 train tickets',
        url=app_url,
        version='1.0.0',
